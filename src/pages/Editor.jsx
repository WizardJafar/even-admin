// src/pages/Editor.jsx (или где у тебя Editor)
...
export default function Editor() {
  ...
  const handleChange = (path, value) => {
    ...
    setData((prev) => {
      ...
      for (let i = 0; i < keys.length - 1; i++) {
        if (!temp[keys[i]] || typeof temp[keys[i]] !== "object") temp[keys[i]] = {};
        temp = temp[keys[i]];
      }
  ...
  const validateI18n = () => {
    ...
    const checkEmpty = (obj) => {
      ...
      if (Array.isArray(obj)) {
        return obj.every((item) => checkEmpty(item));
      }

      if (typeof obj === "object") {
        for (const key in obj) {
          if (!checkEmpty(obj[key])) return false;
        }
        return true;
      }
  ...
  // ---------- renderFields (string/object/array) ----------
  ...
  const getFieldLabel = (key) => {
    const labels = {
      ...
      cta: "CTA matni",
      topTag: "Yuqori yozuv",
      badgeTitle: "Badge sarlavhasi",
      badgeText: "Badge matni",
    };
    return labels[String(key)] || prettyKey(key);
  };

  const renderFields = (value, path = "", depth = 0, isRussian = false) => {
      ...
      return (
        <div className="mb-5 group" key={path}>
          <label className="block text-sm font-semibold text-gray-300 mb-2 transition-colors group-focus-within:text-red-400 flex items-center gap-2">
            ...
            {displayKey}
            {langLabel}
            <span className="text-xs font-normal text-gray-500 ml-1">({path})</span>
          </label>
          ...
          <textarea
            ...
            placeholder={`${displayKey.toLowerCase()} kiriting...`}
          />
        </div>
      );
    }
    ...
    if (Array.isArray(value)) {
      const title = prettyKey(path.split(".").at(-1) || "Array");
      return (
        <div
          className={`mb-6 ${
            depth === 0
        ...
        >
          <h3 className="font-bold text-gray-100 mb-4 flex items-center text-lg">
            ...
            {title}
            {langLabel}
            <span className="text-xs font-normal text-gray-500 ml-2">({path})</span>
          </h3>

          <div className="space-y-4">
            {value.map((item, idx) => {
              const itemPath = path ? `${path}.${idx}` : String(idx);
              return (
                <div
                  key={itemPath}
                  className="rounded-xl border border-gray-700 bg-gray-800/50 p-4"
                >
                  <div className="text-gray-300 font-semibold mb-3">#{idx + 1}</div>
                  {renderFields(item, itemPath, depth + 1, isRussian)}
                </div>
              );
            })}
          </div>
    ...
    if (isPlainObject(value)) {
      const key = path.split(".").at(-1) || "Section";
      ...
      return (
        ...
        >
          ...
          {Object.entries(value).map(([k, v]) => {
            ...
            return renderFields(v, nextPath, depth + 1, isRussian);
          })}
        </div>
      );
    }
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="flex-1">
              ...
              <p className="text-gray-300 text-lg leading-relaxed max-w-3xl">
                Bu yerda siz saytdagi barcha matnlarni tahrirlashingiz mumkin.
                O'zgarishlar ikki til uchun qo'llaniladi:{" "}
                <span className="font-semibold text-gray-100">rus tili</span> va{" "}
                <span className="font-semibold text-gray-100">o'zbek tili</span>.
                Ikkala tildagi barcha maydonlarni to'ldiring va "Saqlash" tugmasini bosing.
              </p>
            </div>
          </div>
        </div>
